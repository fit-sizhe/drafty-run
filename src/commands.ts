...
export async function startSessionHandler(context: vscode.ExtensionContext) {
    ...
    if (pythonAdapter && 'startProcessForDoc' in pythonAdapter) {
        (pythonAdapter as any).startProcessForDoc(
            mdFullPath, 
            pythonPath
        );
    }
...
export async function runBlockHandler(context: vscode.ExtensionContext, range: vscode.Range) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        return;
    }
...
export async function terminateBlockHandler(context: vscode.ExtensionContext, range: vscode.Range) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        return;
    }
